  ...
  listen(options = {}, done = () => {}) {
    const onConnection = (socket) => {
      ...
      socket.on('data', (data) => {
        ...
        for (let req of clientRequests) {
          ...
          async function nextMiddleware(i) {
            const result = await fns[i](req.data);
            if (fns.length - 1 === i) {
    ...
    return new Promise((resolve, reject) => {
      net.createServer(onConnection)
        .listen(options.port || this.PORT, options.host || this.HOST, (err) => {
          if (err) {
            done(err);
            return reject(err)
          }
        ...
        responses.forEach((response) => {
          if (response._cmd && response._cmd === 'ON_CONNECTION') {
            ...
            response.data.forEach((fnName) => {
              const path = fnName.split('.');
              let pointer = service;
              for(let i = 0; i < path.length-1; i++) {
                pointer[path[i]] = pointer[path[i]] || {};
                pointer = pointer[path[i]];
              }
              pointer[path[path.length-1]] = call.bind(client, fnName);
            });
            resolve(service);
            return cb(service);
          }
